...
/**
 * 文件类型的正则表达式和对应的 Class Name
 */
Utils.fileTypeRegexs = [
    {regex: /(\.|\/)(gif|jpe?g|png)$/i, className: 'is-image'},
    {regex: /(\.|\/)(rar)$/i, className: 'is-rar'},
    {regex: /(\.|\/)(zip)$/i, className: 'is-zip'},
    {regex: /(\.|\/)(pdf)$/i, className: 'is-pdf'},
    {regex: /(\.|\/)(mp4)$/i, className: 'is-mp4'}
];

/**
 * 根据文件名的后缀返回对应样式的 Class Name
 */
Utils.getFileTypeClassName = function(fileName) {
    for (var i = 0; i < Utils.fileTypeRegexs.length; ++i) {
        if (Utils.fileTypeRegexs[i].regex.test(fileName)) {
            return Utils.fileTypeRegexs[i].className;
        }
    }
