 ...
 * 提供了多个变种的 ok() 和 fail() 方法简化创建 com.xtuer.bean.Result 对象。
 *
 * 虽然同一个请求在不同情况下返回的 com.xtuer.bean.Result 中的 data 类型可能不同，例如 com.xtuer.bean.Result<User> findUserByName(String name)，
 * 查询到用户时返回 com.xtuer.bean.Result 中 data 是 User 对象，查询不到用户时可返回 com.xtuer.bean.Result 中 data 是 String 对象，不过没关系，
