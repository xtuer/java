...
String REST_TOPICS_BY_ID    = "/rest/topics/{topicId}";
String REST_TOPIC_QUESTIONS = "/rest/topics/{topicId}/questions"; // topic 下的 question
String REST_QUESTIONS       = "/rest/questions"; // 用于创建以及列出所有的 Question
String REST_QUESTIONS_BY_ID = "/rest/questions/{questionId}"; // 用于更新、删除 Question
```
...
```java
// 字符串的 MD5
CommonUtils.md5("your text");

// 文件的 MD5
CommonUtils.md5(new File("file path"));
```
...
使用 yaml 格式进行配置，位于 **config/config.yml**，支持中文，注释（可以是单行，也可以是在某一行的尾部），自动的把首尾的空格去掉，参考 DemoController 中 globalConfig 的用法
...
需要排队执行的任务可以教由任务队列完成，可以创建多个任务队列，默认已经创建了一个 singleTaskQueue。
