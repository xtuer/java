...
public class RepoFileService {
     ...
     * @param baseName 如果 baseName 不为空，则使用它作为移动后文件的名字，而不是文件原来的名字
     * @param dirs 文件夹名的数组
     *
     * @return 返回移动文件到仓库后得到的 url 或者非临时文件返回原 url
     */
    public String moveTempFileToRepo(String url, String baseName, String... dirs) {
        ...
        if (tempFileService.isTempFileUrl(url)) {
            ...
            return this.moveFileToRepo(tempFile, baseName, dirs);
        } else {
     ...
     */
    public String moveFileToRepo(File srcFile, String baseName, String... destDirs) {
        ...
        String filename = StringUtils.isBlank(baseName) ? srcFilename : baseName + "." + FilenameUtils.getExtension(srcFilename);
    ...
    public File getRepoFile(String repoPath) {
        return new File(repoDirectory, repoPath);
    }
