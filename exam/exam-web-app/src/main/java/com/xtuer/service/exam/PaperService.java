...
public class PaperService extends BaseService {
    ...
    public long upsertPaper(Paper paper) {
        ...
        // 8. 统计可作答题目数量
        // 9. 插入或者更新试卷表 exam_paper (试卷自身的数据)
        ...
        // [8] 统计可作答题目数量
        int questionCount = 0;
        Map<Long, Question> questions = getAllQuestionsOfPaper(paper);
        for (Question question : questions.values()) {
            if (question.isDeleted()) {
                continue;
            }

            if (question.isObjective() || question.isSubjective()) {
                questionCount++;
            }
        }
    ...
    public void ensurePaperId(Paper paper) {
        ...
        // [1] 确保试卷的 ID
        if (Utils.isInvalidId(paper.getId())) {
            paper.setId(nextId());
        }
    ...
    private boolean canQuestionScoring(Question question) {
        return question.isObjective() || question.isSubjective();
    }
    ...
    public List<Question> getSubjectiveQuestionsOfPaper(Paper paper) {
        Map<Long, Question> questions = getAllQuestionsOfPaper(paper);
        return questions.values().stream().filter(Question::isSubjective).collect(Collectors.toList());
    }
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    @CacheInvalidate(name = CacheConst.CACHE, key = CacheConst.KEY_PAPER_ID)
    public void deletePaper(long paperId) {
        log.info("[开始] 删除试卷 {}", paperId);

        paperMapper.deletePaper(paperId);

        log.info("[结束] 删除试卷 {}", paperId);
    }
}
