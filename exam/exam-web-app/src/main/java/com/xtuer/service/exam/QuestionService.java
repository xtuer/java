...
import com.xtuer.bean.exam.QuestionOption;
import com.xtuer.mapper.exam.PaperMapper;
import com.xtuer.mapper.exam.QuestionMapper;
...
public class QuestionService extends BaseService {
    ...
    public List<Question> treefyQuestions(List<Question> questions) {
        ...
        List<Question> topQuestions = questions.stream().filter(q -> Utils.isInvalidId(q.getParentId())).collect(Collectors.toList());
        List<Question> subQuestions = questions.stream().filter(q -> Utils.isValidId(q.getParentId())).collect(Collectors.toList());
    ...
    public void appendQuestionOption(Question question, QuestionOption option) {
        ...
        for (QuestionOption o : question.getOptions()) {
            if (Utils.isValidId(option.getId()) && o.getId() == option.getId()) {
                found = true;
                break;
            }
        }
    ...
    public void appendSubQuestion(Question question, Question subQuestion) {
        ...
        for (Question q : question.getSubQuestions()) {
            if (Utils.isValidId(subQuestion.getId()) && q.getId() == subQuestion.getId()) {
                found = true;
                break;
            }
        }
    ...
    public void ensureQuestionId(Question question) {
        ...
        // [1] 确保题目的 ID
        if (Utils.isInvalidId(question.getId())) {
            question.setId(nextId());
        }

        for (QuestionOption option : question.getOptions()) {
            // [1] 确保选项的 ID
            if (Utils.isInvalidId(option.getId())) {
                option.setId(nextId());
            }
    ...
    /**
     * 判断传入的题目类型 questionType 是否客观题类型
     *
    ...
    /**
     * 判断传入的题目类型 questionType 是否主观题类型
     *
     * @param questionType 题目类型
