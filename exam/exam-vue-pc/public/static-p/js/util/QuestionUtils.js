...
export default class QuestionUtils {
    ...
    static createQuestion(type, stem = '请输入题干', purpose = 0) {
        ...
        if (type === QUESTION_TYPE.SINGLE_CHOICE || type === QUESTION_TYPE.MULTIPLE_CHOICE) {
            ...
            // [3] 如果是判断题，则增加 2 个默认选项
            QuestionUtils.appendQuestionOption(question, '正确');
            QuestionUtils.appendQuestionOption(question, '错误');
        } else if (type === QUESTION_TYPE.ESSAY) {
            // [4] 如果是问答题，则创建 1 个选项，用于存储问答题的答案
            QuestionUtils.appendQuestionOption(question);
        } else if (type === QUESTION_TYPE.STAR) {
            // [5] 如果是星级题，则创建 1 个选项，用于存储选值的值
            QuestionUtils.appendQuestionOption(question);
        }
    ...
    static updateQuestionScores(questions) {
        ...
        // [3] 计算总分: 所有可作答题目的满分之和
        let totalScore = 0;
        QuestionUtils.answerableQuestions(questions).filter(q => !q.deleted).forEach(q => {
    ...
    static isScoreGroupQuestion(question) {
        // 在题型题上给分 (每题得分)，包括的题型有: 单选题、多选题、判断题、填空题
    ...
    static isObjectiveQuestion(question) {
        if (question.type === QUESTION_TYPE.SINGLE_CHOICE
                ...
                || question.type === QUESTION_TYPE.STAR) {
            return true;
        }
     ...
     *
     * @param {JSON} question 题目
     * @return {Boolean} 作答过返回 true，否则返回 false
     */
    static isQuestionAnswered(question) {
        // 1. 单选题、多选题、判断题: 有任何选项选中则返回 true
        // 2. 填空题、问答题的任何一个选项的 answer 非空则返回 true
        // 3. 其他情况返回 false

        // [1] 客观题: 单选题、多选题、判断题: 有任何选项选中则返回 true
        if (QuestionUtils.isObjectiveQuestion(question)) {
        ...
        // [2] 主观题: 填空题、问答题的任何一个选项的 answer 非空则返回 true
        if (QuestionUtils.isSubjectiveQuestion(question)) {
            for (let option of question.options) {
                if (option.answer) {
                    return true;
                }
    ...
    /**
     * 判断题目是否可作答题目 (客观题和主观题为可作答题目，题型题、复合题 [不包括他的小题] 为不可作答题目)
     *
     * @param {JSON}} question 题目
     */
    static isAnswerableQuestion(question) {
        return QuestionUtils.isObjectiveQuestion(question) || QuestionUtils.isSubjectiveQuestion(question);
    }
    ...
    static answerableQuestions(questions) {
        ...
        questions.forEach(q => {
            if (QuestionUtils.isObjectiveQuestion(q) || QuestionUtils.isSubjectiveQuestion(q)) {
                qs.push(q);
    ...
    /**
     * 获取 questions 中的客观题 (包括复合题的小题)
     *
     * @param {Array} questions 题目的数组
     * @return {Array} 返回客观题的数组
     */
    static getObjectiveQuestions(questions) {
        return QuestionUtils.answerableQuestions(questions).filter(q => QuestionUtils.isObjectiveQuestion(q));
    }

    /**
     * 获取 questions 中的主观题 (包括复合题的小题)
     *
     * @param {Array} questions 题目的数组
     * @return {Array} 返回主观题的数组
     */
    static getSubjectiveQuestions(questions) {
        return QuestionUtils.answerableQuestions(questions).filter(q => QuestionUtils.isSubjectiveQuestion(q));
    }
}
